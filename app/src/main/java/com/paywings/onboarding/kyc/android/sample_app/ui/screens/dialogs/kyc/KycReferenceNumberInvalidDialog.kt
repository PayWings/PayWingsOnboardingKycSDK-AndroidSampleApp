...
@Composable
fun Preview_KycReferenceNumberInvalidDialog() {
    val kycReferenceNumberInvalidDialogState = rememberMaterialDialogState(initialValue = true)
...
@Composable
private fun KycReferenceNumberInvalidDialogContent(){
    Column(modifier = Modifier
        .fillMaxWidth()
        .padding(all = MaterialTheme.shapes.dialogEdgeDefaultPadding)) {
        ScreenTitle(
            modifier = Modifier.align(alignment = Alignment.CenterHorizontally),
            stringResId = R.string.kyc_reference_number_invalid_dialog_title
        )
        SpacerDialogTitleBody()
        Text(
            style = MaterialTheme.typography.bodyMedium,
            text = stringResource(id = R.string.kyc_reference_number_invalid_dialog_description)
        )
    }
}