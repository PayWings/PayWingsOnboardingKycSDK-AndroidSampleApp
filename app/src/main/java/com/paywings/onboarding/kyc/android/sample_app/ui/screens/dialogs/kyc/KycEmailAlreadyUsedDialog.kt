...
@Composable
fun Preview_KycEmailAlreadyUsedDialog() {
    val kycEmailAlreadyUsedDialogState = rememberMaterialDialogState(initialValue = true)
...
fun KycEmailAlreadyUsedDialog(
    ...
    MaterialDialog(
        ...
        KycEmailAlreadyUsedDialogContent()
    }
}

@Composable
private fun KycEmailAlreadyUsedDialogContent(){
    Column(modifier = Modifier
        .fillMaxWidth()
        .padding(all = MaterialTheme.shapes.dialogEdgeDefaultPadding)) {
        ScreenTitle(
            modifier = Modifier.align(alignment = Alignment.CenterHorizontally),
            stringResId = R.string.kyc_email_already_used_dialog_title
        )
        SpacerDialogTitleBody()
        Text(
            style = MaterialTheme.typography.bodyMedium,
            text = stringResource(id = R.string.kyc_email_already_used_dialog_description)
        )
    }
}